        ...
        function isValidated(elements, captcha) {
            ...
            if (elements.length) {
                ...
                if (captcha && captcha.length) {
                    ...
                    if (!captchaToken) {
                        errors++;
                        captcha.siblings(".form-validation").text("Please complete the reCAPTCHA").parent().addClass("has-error");
                    } else {
                        captcha.siblings(".form-validation").text("").parent().removeClass("has-error");
                    }
                }
			...
			for (i = 0; i < plugins.rdMailForm.length; i++) {
				...
				$form.attr('novalidate', 'novalidate').ajaxForm({
					...
					success: function (result) {
						...
						if (result.status === "MF000") {
							if (output.hasClass("snackbars")) {
								output.html('<p><span class="icon text-middle mdi mdi-check icon-xxs"></span><span>' + msg[result.status] + '</span></p>');
							} else {
								output.addClass("active success");
							}
						} else {
							if (output.hasClass("snackbars")) {
								output.html(' <p class="snackbars-left"><span class="icon icon-xxs mdi mdi-alert-outline text-middle"></span><span>' + msg[result.status] + '</span></p>');
							} else {
